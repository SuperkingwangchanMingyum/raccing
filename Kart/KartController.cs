...
public class KartController : KartComponent
{
    ...
    [Header("Tire Rotation Settings")]
    [SerializeField] private Axis tireYawAxis = Axis.Y;  // 조향 축 (기본: Y축)
    [SerializeField] private Axis tireRollAxis = Axis.X;  // 구름 회전 축 (기본: X축)
    ...
    public bool CanDrive => HasStartedRace && !HasFinishedRace && !IsSpinout && !IsBumped && !IsBackfire && !IsTimeUp;
    ...
    [Networked] public TickTimer HopTimer { get; set; }
    ...
    [Networked] public NetworkBool IsTimeUp { get; set; }
    ...
    public override void FixedUpdateNetwork()
    {
        ...
        if (IsTimeUp)
        {
            AppliedSpeed = Mathf.Lerp(AppliedSpeed, 0, Runner.DeltaTime * deceleration * 2f);
            RefreshAppliedSpeed();
        }
        else if (CanDrive)
        {
    ...
    {
        // 조향(Yaw) - 전륜만 적용, tireYawAxis 설정에 따라 회전
        ...
        // 구름 회전 - tireRollAxis 설정에 따라 회전 (모든 바퀴)
        if (CanDrive)
        {
            float rotationSpeed = 90 * Time.deltaTime * AppliedSpeed * 0.5f;
            
            // 선택된 축에 따라 회전 적용
            RotateTireAroundAxis(tireFL, tireRollAxis, rotationSpeed);
            RotateTireAroundAxis(tireFR, tireRollAxis, rotationSpeed);
            RotateTireAroundAxis(tireBL, tireRollAxis, rotationSpeed);
            RotateTireAroundAxis(tireBR, tireRollAxis, rotationSpeed);
        }
    }

    private void RotateTireAroundAxis(Transform tire, Axis axis, float speed)
    {
        switch (axis)
        {
            case Axis.X:
                tire.Rotate(speed, 0, 0);
                break;
            case Axis.Y:
                tire.Rotate(0, speed, 0);
                break;
            case Axis.Z:
    ...
    private void StopDrifting()
    {
        ...
        BoostEndTick = BoostTierIndex == 0
            ? -1
            : Runner.Tick + (int)(driftTiers[BoostTierIndex].boostDuration / Runner.DeltaTime);
